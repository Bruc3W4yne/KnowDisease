...
class CoTFormFillSignature(dspy.Signature):
    """
        You are an expert biomedical information-extraction model with deep understanding of medical terminology and research papers.

        TASK: Extract specific medical terms for the field "{answer_field_name}"
        ...
        FIELD SPECIFICATION
        ------------------
        Field Name: {answer_field_name}
        Description: {answer_field_description}

        EXTRACTION METHODOLOGY
        ---------------------
        1. SCAN: Identify ALL candidate terms related to the field
        2. FILTER: Select only terms explicitly stated in context
        3. PRIORITIZE: Rank by:
        - Specificity (e.g., "ipilimumab 3mg/kg" > "immunotherapy")
        - Clinical relevance to the field
        - Frequency of mention
        4. SELECT: Choose up to 3 MOST relevant terms

        CRITICAL RULES
        -------------
        • Extract terms EXACTLY as written (preserve doses, subtypes, qualifiers)
        • Evidence MUST be verbatim sentences from context
        • If <3 relevant terms exist, return only what's available
        • If 0 relevant terms exist, return empty arrays

        OUTPUT FORMAT
        ------------
        {
        "reasoning": "Your step by step reasoning for why you selected the terms you did.",
        "final_answer": ["<exact_term_1>", "<exact_term_2>", "<exact_term_3>"],
        "evidence": {
            "<exact_term_1>": "<complete sentence containing term 1>",
            "<exact_term_2>": "<complete sentence containing term 2>",
            "<exact_term_3>": "<complete sentence containing term 3>"
            }
        }
...
class FieldFiller(dspy.Module):
    ...
    def forward(self, prompt_input, context, field_type):
        ...
        if hasattr(self, 'use_cot') and self.use_cot:
            # CoT mode - try once, fail gracefully
            try:
                ...
                print(f"WARNING: CoT generation failed: {e}")
...
class SequentialFormFiller(dspy.Module):
    ...
    def __init__(self,
        ...
        if use_cot and cot_schema:
            ...
            self.max_tokens = max(max_tokens, 3500)  # Increase for CoT
        elif use_json_constraints and not use_cot:
            self.signature = SimpleFormFillSignature
            self.max_tokens = max(max_tokens, 1000)   # Increase for JSON
        elif listify_form:
